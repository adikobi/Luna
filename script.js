...
function initializeApp() {
    ...
    // --- DATA FUNCTIONS ---
    const defaultPeople = [
        { id: 1, name: 'מאיה', image: 'https://i.pravatar.cc/150?u=maya', moments: [{date: "2025-08-25", text: "רגע לדוגמה"}] },
    ...
    const loadData = () => {
        ...
        if (data && data.length > 2) { return JSON.parse(data); }
    ...
    const renderPersonDetail = (personId) => {
        ...
        const avatarHTML = person.image ? `<img src="${person.image}" alt="${person.name}" class="detail-avatar-img">` : `<div class="default-avatar detail-avatar-icon" style="background-color: ${themeColors[allPeople.indexOf(person) % themeColors.length]}"><i class="fas fa-user"></i></div>`;
        ...
        document.body.innerHTML = `<header class="app-header"><button id="back-to-grid" class="back-button">&larr; חזרה</button><h1>${person.name}</h1><div style="width: 50px;"></div></header><main id="app-main"><div class="person-detail-header">${avatarHTML}</div><section class="moments-section"><h2>הוסף רגע חדש</h2><form id="add-moment-form"><textarea id="moment-text-input" placeholder="כתוב כאן משהו..." required></textarea><button type="submit">שמור רגע</button></form><h2>רגעים</h2><ul class="moments-list">${person.moments.length > 0 ? momentsHTML : '<p class="no-results">אין עדיין רגעים.</p>'}</ul></section></main>`;
